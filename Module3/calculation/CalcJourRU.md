...
> Структура задачи: **T<sub>i</sub>(P,e,D)**, где **i** — номер задачи **T** данного набора задач; **P** — *период (period)*, или время между двумя последовательными активациями задачи; **e** — *время выполнения (execution time)*, или время, необходимое для полного (корректного) завершения задачи; **D** — *крайний срок (deadline)*, или предельное время, к которому задача должна быть завершена.  
...
где **f** — размер кадра; **P<sub>i</sub>** — период задачи **i**; **D<sub>i</sub>** — крайний срок выполнения задачи **i**; **GCD(P<sub>i</sub>, f)** — **наибольший общий делитель** (*greatest common divisor, GCD*) для значений периода и кадра.
...
Согласно требованию, искомое значение кадра должно делить гиперпериод **H** нацело.

**Пример «ручного» поиска делителей для числа 660**

Делители числа 660 можно получить, разложив исходное число на простые множители:
